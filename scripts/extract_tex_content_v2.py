...
class TikzNetworkExtractor(BaseExtractor):
    ...
    def process(self) -> List[Dict[str, Any]]:
        ...
        print(f"tikz-network: {len(items)} items")
        return items
...
class ChemfigExtractor(BaseExtractor):
    ...
    def process(self) -> List[Dict[str, Any]]:
        ...
        print(f"chemfig: {len(items)} items")
        return items
...
class CircuitikzExtractor(BaseExtractor):
    ...
    def process(self) -> List[Dict[str, Any]]:
        ...
        print(f"circuitikz: {len(items)} items")
        return items
...
class TkzEuclideExtractor(BaseExtractor):
    ...
    def process(self) -> List[Dict[str, Any]]:
        ...
        print(f"tkz-euclide: {len(items)} items")
        return items
...
class StandardExtractor(BaseExtractor):
    ...
    def process(self) -> List[Dict[str, Any]]:
        ...
        print(f"{self.package_name}: {len(items)} items")
        return items
...
def main():
    ...
    # 手册配置
    packages = [
        ("tikz-network", "tikz-network-manual"),
        ("chemfig", "chemfig-manual"),
        ("circuitikz", "circuitikz-manual"),
        ("tkz-euclide", "tkz-euclide-manual"),
        ("pgfplots", "pgfplots-manual"),
        ("tikz-pgf", "tikz-pgf-manual")
    ]
    ...
    print("=" * 60)
    print("LaTeX Manual Knowledge Extraction V2")
    print("=" * 60)

    for package_name, manual_dir_name in packages:
        print(f"\n=== Processing {package_name} ===")

        manual_dir = manual_base / manual_dir_name
        if not manual_dir.exists():
            print(f"Warning: {manual_dir} not found, skipping")
            continue
    ...
    print("=" * 60)
    for package, count in stats.items():
        if package != "total":
            print(f"{package:20s}: {count:6d} items")
    print("-" * 60)
    print(f"{'Total':20s}: {stats['total']:6d} items")
